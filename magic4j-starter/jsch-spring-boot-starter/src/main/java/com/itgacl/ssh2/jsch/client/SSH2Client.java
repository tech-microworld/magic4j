package com.itgacl.ssh2.jsch.client;

import com.itgacl.ssh2.jsch.bean.JschUploadResult;
import com.itgacl.ssh2.jsch.util.FileUtil;
import com.itgacl.ssh2.jsch.util.GUIDUtil;
import com.itgacl.ssh2.jsch.util.MD5Util;
...
public class SSH2Client {
    ...
    public List<JschUploadResult> uploadFiles(Map<String,InputStream> inputStreamMap, String remoteSavePath, SftpProgressMonitor progressMonitor) throws SftpException {
        ...
        for(Map.Entry<String,InputStream> src:inputStreamMap.entrySet()){
            ...
            try {
                byte[] bytes = FileUtil.input2ByteArray(inputStream);//inputStream只能读取一次
                jschUploadResult.setFileMd5(MD5Util.getBytesMD5(bytes));//计算MD5
