...
public class AliyunOssStorageServiceImpl extends BaseStorageService {
    ...
    private static final String STORAGE_TYPE = StorageTypeEnum.ALIYUN_OSS.getDescription();
    ...
    public UploadResult uploadFile(InputStream inputStream, String fileName, String savePath) {
        ...
        try{
            ...
            String fileUrl = ossStorageService.upload(bytes,savePath);//上传文件
            uploadResult.setFileUrl(fileUrl);
            uploadResult.setSavePath(ossStorageService.getSavePath(fileUrl));
    ...
    public UploadResult uploadFile(byte[] fileByte, String fileName, String savePath) {
        ...
        try{
            ...
            if(StrUtil.isNotEmpty(savePath)){
                savePath = savePath+"/"+newFileName;
                String fileUrl = ossStorageService.upload(fileByte,savePath);
                uploadResult.setSavePath(ossStorageService.getSavePath(fileUrl));
                uploadResult.setFileUrl(fileUrl);
            }else {
                String fileUrl = ossStorageService.upload(fileByte,newFileName);
                uploadResult.setSavePath(ossStorageService.getSavePath(fileUrl));
                uploadResult.setFileUrl(fileUrl);
            }
    ...
    public int deleteFile(String fileUrl) {
        try {
            ossStorageService.deleteFile(fileUrl);
            return 0;
        }catch (Exception e){
            log.error(e.getMessage(),e);
            return -1;
    ...
    public Map<String, Integer> deleteFile(List<String> listFileUrl) {
        ossStorageService.deleteFile(listFileUrl);
        return null;
    }
}
