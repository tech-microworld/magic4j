...
public class QiniuKodoStorageServiceImpl extends BaseStorageService {
    ...
    private static final String STORAGE_TYPE = StorageTypeEnum.QINIU_KODO.getDescription();
    ...
    public UploadResult uploadFile(InputStream inputStream, String fileName, String savePath) {
        ...
        try{
            ...
            String fileUrl = kodoStorageService.upload(bytes,savePath+"/"+newFileName);//上传文件
            uploadResult.setFileUrl(fileUrl);
            uploadResult.setSavePath(kodoStorageService.getSavePath(fileUrl));
    ...
    public UploadResult uploadFile(byte[] fileByte, String fileName, String savePath) {
        ...
        try{
            ...
            String newFileName = GUIDUtil.genRandomGUID()+"."+ext;
            uploadResult.setNewFileName(newFileName);
            String fileUrl = kodoStorageService.upload(fileByte,savePath+"/"+newFileName);
            uploadResult.setFileUrl(fileUrl);
            uploadResult.setSavePath(kodoStorageService.getSavePath(fileUrl));
    ...
    public int deleteFile(String fileUrl) {
        try {
            kodoStorageService.deleteFile(fileUrl);
            return 0;
        }catch (Exception e){
            log.error(e.getMessage(),e);
            return -1;
    ...
    public Map<String, Integer> deleteFile(List<String> listFileUrl) {
        kodoStorageService.deleteFile(listFileUrl);
        return null;
    }
}
